...
def render_info_sections(T):
    ...
    with st.expander(T["expander_imams"], expanded=False):
        ...
        if imams:
            ...
            st.info("لا توجد بيانات للأئمة. تأكد من وجود ملف data/imams.json")
        ...
        if countries:
            ...
            for i, c in enumerate(countries):
                with cols_country[i % 3]:
                    ...
                    madhab_data = c.get("madhab", "")
                    if isinstance(madhab_data, list):
                        madhab_names = []
                        for m in madhab_data:
                            if m in MADHHAB_NAMES:
                                madhab_names.append(MADHHAB_NAMES[m][lang])
                        madhab = " & ".join(madhab_names)
                    else:
            ...
            st.info("لا توجد بيانات للدول. تأكد من وجود ملف data/countries.json")
    ...
    with st.expander(T["expander_glossary"], expanded=False):
        ...
        if glossary:
            ...
            st.info("لا توجد بيانات للمصطلحات. تأكد من وجود ملف data/glossary.json")
    ...
    with st.expander(T["legal_sources"], expanded=False):
        ...
        if sources:
            ...
            st.info("لا توجد بيانات لمصادر التشريع. تأكد من وجود ملف data/legal_sources.json")
    ...
    with st.expander(T["usul"], expanded=False):
        ...
        if usul:
            ...
            st.info("لا توجد بيانات لأصول الاستدلال. تأكد من وجود ملف data/usul.json")
    ...
    with st.expander(T["rules_title"], expanded=False):
        ...
        if rules:
            ...
            st.info("لا توجد بيانات للقواعد. تأكد من وجود ملف data/rules.json")
